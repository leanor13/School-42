...
/*   Updated: 2024/03/27 18:36:45 by yioffe           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	input_instruction(void)
{
	...
	ft_putstr_fd("    - For Mandelbrot set send 'm' or 'Mandelbrot'\n",
		STDERR_FILENO);
	ft_putstr_fd("    - For Julia set send 'j' or 'Julia', and then "
		"(optional) 2 double values in format: xx.xx and yy.yy. Both "
		"values should be in range (-2.0, 2.0)\n", STDERR_FILENO);
	ft_putstr_fd("    - For Tricorn set send 't' or 'Tricorn'\n",
		STDERR_FILENO);
	ft_putstr_fd("2. Adjust iteration number (optional):\n", STDERR_FILENO);
	ft_putstr_fd("    If you wish to adjust number of iterations, "
		"send i_xxx, where x is digit. Example: i_100, maximum i_999, "
		"minimum i_2\n", STDERR_FILENO);
	ft_putstr_fd("    High iterations number may and wil cause slowness "
		"of the program\n", STDERR_FILENO);
